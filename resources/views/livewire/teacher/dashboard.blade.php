<div x-data="{ step: @entangle('step') }" class="space-y-8">
   ...
   <div class="bg-blue-500 rounded-lg p-6 text-white transition duration-500">
    ...
    <p class="text-blue-100">
        ...
        <button class="bg-gray-700 text-white px-2 py-1 rounded">Back</button>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        @if ($step === 'confirm')
            <div x-transition class="flex flex-col items-center border p-5 rounded-lg shadow bg-white animate-fadeIn">
        ...
        @if ($step === 'level')
            @foreach ($levels as $level)
                <button wire:click="updateLevel({{ $level->id }})" x-transition
                    class="flex flex-col items-center gap-2 border-2 border-gray-300 p-5 rounded-lg 
