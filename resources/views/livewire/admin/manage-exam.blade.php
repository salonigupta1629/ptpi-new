<div class="container mx-auto px-4 py-8 max-w-7xl">
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-left text-sm font-semibold text-gray-600 uppercase tracking-wider">Category</th>
                    ...
                    @forelse ($examSets as $exam)
                        <tr class="hover:bg-gray-50 transition-colors duration-200">
                            <td class="px-6 py-4">
                                ...
                                <div class="text-sm text-gray-500 line-clamp-2">{{ Str::limit($exam->description, 60) }}</div>
                            </td>
    ...
    @if ($isModalOpen)
        <div class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
            <div class="flex items-center justify-center min-h-screen px-4 py-8">
                <div class="fixed inset-0 bg-gray-900 bg-opacity-60 backdrop-blur-sm transition-opacity duration-300" wire:click="closeModal"></div>

                <div class="relative w-full max-w-3xl p-8 mx-auto bg-white rounded-2xl shadow-2xl transform transition-all duration-300 scale-100">
                    <div class="absolute top-4 right-4">
                        <button wire:click="closeModal" class="text-gray-400 hover:text-gray-600 transition-colors duration-200 focus:outline-none">
                        ...
                        <form wire:submit.prevent="storeOrUpdate" class="space-y-6">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    ...
                                    @error('name') <span class="text-red-500 text-xs mt-1.5">{{ $message }}</span> @enderror
                                </div>
    ...
    @error('category_id') <span class="text-red-500 text-xs mt-1.5">{{ $message }}</span> @enderror
</div>
    ...
    @error('subject_id') <span class="text-red-500 text-xs mt-1.5">{{ $message }}</span> @enderror
</div>
    ...
    @error('level_id') <span class="text-red-500 text-xs mt-1.5">{{ $message }}</span> @enderror
</div>
                                ...
                                <div>
                                    ...
                                    <input type="number" wire:model="total_marks" id="total_marks" 
                                        class="w-full p-3 border border-gray-200 rounded-xl shadow-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors duration-200">
                                    @error('total_marks') <span class="text-red-500 text-xs mt-1.5">{{ $message }}</span> @enderror
                                </div>
                                ...
                                <div>
                                    ...
                                    <input type="number" wire:model="duration" id="duration" 
                                        class="w-full p-3 border border-gray-200 rounded-xl shadow-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors duration-200">
                                    @error('duration') <span class="text-red-500 text-xs mt-1.5">{{ $message }}</span> @enderror
                                </div>
                                ...
                                <div>
                                    ...
                                    @error('type') <span class="text-red-500 text-xs mt-1.5">{{ $message }}</span> @enderror
                                </div>
                                ...
                                <div>
                                    ...
                                    @error('status') <span class="text-red-500 text-xs mt-1.5">{{ $message }}</span> @enderror
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                @error('description') <span class="text-red-500 text-xs mt-1.5">{{ $message }}</span> @enderror
                            </div>
