...
class AddQuestion extends Component
{
    ...
    protected $rules = [
        ...
        // Allow manual Hindi input for LaTeX-heavy questions
    ...
    public function createQuestion()
    {
        ...
        // Use manual Hindi input if provided; otherwise, attempt translation
        $questionTextHi = $this->question_text_hi ?: $this->translateText($this->stripLatex($this->question_text));
        $optionsHi = $this->options_hi[0] ? $this->options_hi : array_map(function ($option) {
            return $option ? $this->translateText($this->stripLatex($option)) : '';
        }, $this->options);
        $solutionHi = $this->solution_hi ?: ($this->solution ? $this->translateText($this->stripLatex($this->solution)) : null);
    ...
    // Helper to strip LaTeX for translation
    private function stripLatex($text)
    {
        // Remove LaTeX markup (basic example, adjust as needed)
